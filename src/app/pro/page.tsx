        ...
        <ProPreviewGate>
          <div className="mb-5">
            ...
            <h2 className="text-3xl font-black text-white">
              Pro V4 Signal Dashboard
            </h2>
            <p className="mt-2 max-w-3xl text-slate-400">
              Pro Signals are filtered to long-only / bullish-only ideas. No
              short signals are shown in the paid signal dashboard.
            </p>
          </div>

          {!bestBullishSignal && (
            <div className="mb-6 rounded-2xl border border-yellow-500/30 bg-yellow-500/10 p-5 text-yellow-100">
              No bullish-only Pro Signals are available right now. The app will
              update automatically when the V4 bot finds a qualifying long idea.
            </div>
          )}

          <div className="grid gap-6 lg:grid-cols-2">
            <PaidCard title="Best Bullish Signal Right Now" isUnlocked={isProPreview}>
              {bestBullishSignal ? (
                <>
                  <p className="text-3xl font-black text-cyan-300">
                    {(bestBullishSignal?.symbol || "WAITING")}
                  </p>
                  <p className="mt-2 text-white">
                    {(bestBullishSignal?.direction || "Waiting")} —{" "}
                    {bestBullishSignal.confidence}% confidence
                  </p>
                  <p className="mt-2 text-sm text-slate-300">
                    Risk: {bestBullishSignal.risk}
                  </p>
                </>
              ) : (
                <p className="text-slate-300">Waiting for bullish signal.</p>
              )}
            </PaidCard>

            <PaidCard title="ML Score" isUnlocked={isProPreview}>
              {bestBullishSignal ? (
                <div>
                  <p className="text-5xl font-black text-cyan-300">
                    {bestBullishSignal.ml_score}
                  </p>
                  <p className="mt-2 text-sm text-slate-300">
                    ML score is part of the paid signal intelligence and is not
                    shown in the free preview.
                  </p>
                </div>
              ) : (
                <p className="text-slate-300">Waiting for ML data.</p>
              )}
            </PaidCard>
          </div>

          <div className="mt-6">
            <PaidCard title="Why this signal?" isUnlocked={isProPreview}>
              {bestBullishSignal ? (
                <div className="grid gap-4 md:grid-cols-3">
                  <div className="rounded-xl bg-slate-950 p-4">
                    <p className="text-sm text-slate-400">Confidence</p>
                    <p className="mt-2 text-2xl font-black text-white">
                      {bestBullishSignal.confidence}%
                    </p>
                  </div>
                  <div className="rounded-xl bg-slate-950 p-4">
                    <p className="text-sm text-slate-400">Momentum</p>
                    <p className="mt-2 text-2xl font-black text-white">
                      {bestBullishSignal.momentum_score}
                    </p>
                  </div>
                  <div className="rounded-xl bg-slate-950 p-4">
                    <p className="text-sm text-slate-400">Pattern</p>
                    <p className="mt-2 text-2xl font-black text-white">
                      {bestBullishSignal.pattern_score}
                    </p>
                  </div>
                  <div className="rounded-xl bg-slate-950 p-4 md:col-span-3">
                    <p className="text-sm text-slate-400">Reasoning</p>
                    <p className="mt-2 text-sm leading-6 text-slate-200">
                      {getSignalReason(bestBullishSignal)}
                    </p>
                  </div>
                </div>
              ) : (
                <p className="text-slate-300">Waiting for signal reasoning.</p>
              )}
            </PaidCard>
          </div>

          <div className="mt-6">
            <PaidCard title="Full Bullish Top 10" isUnlocked={isProPreview}>
              <div className="overflow-hidden rounded-xl border border-slate-800">
                <table className="w-full text-left text-sm">
                  ...
                  <tbody>
                    {(bullishSignals || []).slice(0, 10).map((signal, index) => (
                      <tr key={`${(signal?.symbol || "WAITING")}-${index}`} className="border-t border-slate-800">
                        <td className="p-3">{index + 1}</td>
                        <td className="p-3 font-bold">{(signal?.symbol || "WAITING")}</td>
                        <td className="p-3">{(signal?.direction || "Waiting")}</td>
                        <td className="p-3">{signal.confidence}%</td>
                        <td className="p-3">{signal.ml_score}</td>
                        <td className="p-3">{signal.risk}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
          ...
          <div className="mt-6 grid gap-6 lg:grid-cols-2">
            <PaidCard title="Open Paper Trades" isUnlocked={isProPreview}>
              <div className="space-y-3">
                {(proData.open_paper_trades || []).length === 0 && (
                  <p className="text-slate-300">No open paper trades right now.</p>
                )}

                {(proData.open_paper_trades || []).map((trade) => (
                  <div key={(trade?.symbol || "WAITING")} className="rounded-xl bg-slate-950 p-4">
                    <p className="font-bold text-white">{(trade?.symbol || "WAITING")}</p>
                    <p className="text-sm text-slate-400">
                      Entry: ${trade.entry} | Current: ${trade.current}
                    </p>
                    <p className="text-sm text-cyan-300">
                      P/L: {trade.unrealized_pl_percent}%
                    </p>
                  </div>
                ))}
              </div>
            ...
            <PaidCard title="Closed Trade Stats" isUnlocked={isProPreview}>
              <div className="grid grid-cols-2 gap-3">
                <StatCard
                  label="Win Rate"
                  value={proData.closed_trade_stats.win_rate}
                />
                <StatCard
                  label="Closed Trades"
                  value={proData.closed_trade_stats.total_closed_trades}
                />
                <StatCard label="Wins" value={proData.closed_trade_stats.wins} />
                <StatCard label="Losses" value={proData.closed_trade_stats.losses} />
              </div>
            </PaidCard>
          </div>

          <div className="mt-8 rounded-2xl border border-yellow-500/30 bg-yellow-500/10 p-5 text-sm leading-6 text-yellow-100">
            <strong>Reminder:</strong> All Pro data is still paper-trading
            research only. Signals are for testing, education, and app
            subscription development. This app does not execute real trades.
          </div>
            </ProPreviewGate>
        </section>
      </section>
